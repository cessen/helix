...
impl InternedRopeLines {
    pub fn new(diff_base: Rope, doc: Rope) -> InternedRopeLines {
        let mut res = InternedRopeLines {
            interned: InternedInput {
                before: Vec::with_capacity(diff_base.len_lines(LINE_TYPE)),
                after: Vec::with_capacity(doc.len_lines(LINE_TYPE)),
                interner: Interner::new(diff_base.len_lines(LINE_TYPE) + doc.len_lines(LINE_TYPE)),
            },
    ...
    fn is_too_large(&self) -> bool {
        ...
        // for large files
        self.doc.len_lines(LINE_TYPE) > MAX_DIFF_LINES
            || self.diff_base.len_lines(LINE_TYPE) > MAX_DIFF_LINES
